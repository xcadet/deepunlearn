...
def train_negrad(
    ...
    for idx, ((input, target), (del_input, del_target)) in enumerate(
        zip(train_loader, cycle(delete_loader))
    ):
...
def train_distill_hide(
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=16, num_workers=0, pin_memory=True, shuffle=True
    )
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=16, num_workers=0, pin_memory=True, shuffle=True
    )
...
def train_bad_teacher(
    epoch, train_loader, delete_loader, module_list, criterion_list, optimizer, opt
):
    """One epoch distillation"""
    # set modules as train()
    for module in module_list:
...
def train_bcu(
    ...
    for (input, target), (del_input, del_target) in zip(
        train_loader, cycle(delete_loader)
    ):
        ...
        if opt.bcu_vec == "logits":
            ...
            loss2, bcu_acc = criterion_list[1](
                logit_s_r, logit_s_d, logit_t_r, logit_t_d
            )
        else:
            loss1 = criterion_list[0](logit_s_r, target)
